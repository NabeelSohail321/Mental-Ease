...
import 'package:mental_ease/Admin/providers/Admin_App_revenue_Provider.dart';
...
class AdminAppFeeRevenueScreen extends StatefulWidget {
  @override
  _AdminAppFeeRevenueScreenState createState() => _AdminAppFeeRevenueScreenState();
}

class _AdminAppFeeRevenueScreenState extends State<AdminAppFeeRevenueScreen> {
  late AdminAppFeeProvider _provider;
  ...
  Future<void> _generatePdf(AdminAppFeeProvider provider) async {
    try {
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (pw.Context context) => [
              ...
              children: [
                if (_appLogo != null)
                  pw.Column(
                      children: [
                        pw.Image(
                          pw.MemoryImage(_appLogo! as services.Uint8List),
                          height: 60,
                        ),
            ...
            // Revenue Trend Chart (simulated as a table)
            pw.Header(
              level: 1,
            ...
            pw.Table.fromTextArray(
              ...
              data: dailyRevenue.entries.map((e) =>
              [e.key, '\$${e.value.toStringAsFixed(2)}']).toList(),
            ),
            pw.SizedBox(height: 20),
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<AdminAppFeeProvider>.value(
      value: _provider,
      child: Scaffold(
        appBar: _buildAppBar(context),
        body: _buildBody(context),
        floatingActionButton: Consumer<AdminAppFeeProvider>(
  ...
  Widget _buildBody(BuildContext context) {
    return Consumer<AdminAppFeeProvider>(
      builder: (context, provider, child) {
  ...
  Widget _buildDateRangePicker(BuildContext context, AdminAppFeeProvider provider) {
    return Card(
      elevation: 4,
  ...
  Widget _buildRevenueChart(BuildContext context, AdminAppFeeProvider provider) {
  ...
  Widget _buildRevenueSummary(BuildContext context, AdminAppFeeProvider provider) {
    final size = MediaQuery.of(context).size;
  ...
  Widget _buildAppointmentList(BuildContext context, AdminAppFeeProvider provider) {
    if (provider.revenues.isEmpty) {
      return Center(
        child: Text(

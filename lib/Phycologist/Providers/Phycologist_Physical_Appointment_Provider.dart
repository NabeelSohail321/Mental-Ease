...
class AppointmentProvider12 extends ChangeNotifier {
  ...
  Stream<List<AppointmentData>> getPhysicalAppointmentsStream() {
    ...
    return FirebaseDatabase.instance
      ...
      //   }
      // }
      // Fetch patient (user) names for each appointment
      for (var appointment in appointments) {
        if (appointment.userId.isNotEmpty) {
          try {
            final userSnapshot = await _usersRef.child(appointment.userId).get();
            if (userSnapshot.exists) {
              final userData = userSnapshot.value as Map<dynamic, dynamic>;
              appointment.patientName = userData['username'] ?? 'Unknown User'; // using doctorName as display
            }
          } catch (e) {
